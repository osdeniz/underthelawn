extends TestBase
## G22: the burn from the player's camera, a strip cut so the scorched earth shows.
func run() -> void:
	suite = "KUL CEKIM"
	min_checks = 1
	SkyTime.set_mode(GameConfig.SKY_MODE_DAY)
	var game: Node = await open("ch10_relay_hill")
	...
	await frames(2)
	await drawn_frame()
	get_viewport().get_texture().get_image().save_png("res://out/ash.png")
	print("[cekim] out/ash.png yazildi")
	ck("kul paleti", GameConfig.active_grass_palette == "ASH", GameConfig.active_grass_palette)
